            ...
            <ScrollView>
                ...
                <View style={authStyle.formContainer}>
                    <View style={{ marginTop: 15 }}>
                        <AppText style={authStyle.text}>Choose Organization</AppText>
                        <View style={authStyle.organizationContainer}>
                            <Button title={'KMC'} width={'50%'}
                                style={selectedOrganization === 'kmc' ? authStyle.active : {}}
                                buttonFontStyle={selectedOrganization === 'kmc' ? authStyle.text : {}}
                                onPress={() => setSelectedOrganization('kmc')}
                            />
                            <Button
                                title={'CBC'}
                                width={'50%'}
                                onPress={() => setSelectedOrganization('cbc')}
                                style={selectedOrganization === 'cbc' ? authStyle.active : {}}
                                buttonFontStyle={selectedOrganization === 'cbc' ? authStyle.text : {}}
                            />
                        </View>
                    </View>
                    ...
                    />
                    {!fetching && !location || (gallery) && <React.Fragment>
                        <AppText>
                            Location
                        </AppText>
                        <Input
                    ...
                    />
                    <AppText style={authStyle.text}>
                        Send complain as anonymous
                    </AppText>
                    <Button
                        title={anonymous ? 'Anoymous mode' : 'Not anonymous mode'}
                        style={{ backgroundColor: 'white', marginTop: 5, marginBottom: 5 }}
                        buttonFontStyle={{ color: Colors.black, fontWeight: '600' }}
                        onPress={() => setAnonymous(!anonymous)}
                    />
                    <View>

                        <AppText
                            style={authStyle.text}
                        >Choose From Gallery</AppText>
                        <ImageInput
                            imageUri={imageUri}
                        ...
                        />
                        <ErrorMessage visible={error} error={error} style={{ textAlign: 'center', marginTop: 30 }} />
                    </View>
                ...
                <View style={authStyle.registerBtn}>
                    {!loading && !fetching ? <Button title='Post'
                        onPress={handleSubmit}
                    /> : <ActivityIndicator color={!fetching ? Colors.black : 'white'} />}
                </View>
            </ScrollView>
        </Screen>
    )
